...
def detect_kwargs_unpacking(
    ...
    for node in ast.walk(tree):
        ...
        if entries is None:
            continue
        _check_call_kwargs(node, call_name, entries, symbol_table, dict_assigns, file_path, findings)
    return findings
...
def _check_call_kwargs(
    ...
    for kw in node.keywords:
        if kw.arg is not None:
            continue
        resolved = _resolve_kwargs_dict(kw.value, symbol_table, dict_assigns)
        if resolved is None:
            continue
...
def _resolve_kwargs_dict(
    ...
    Returns ``None`` when the expression cannot be resolved.
    """
...
def _collect_dict_assigns(tree: ast.Module) -> dict[str, dict[str, str]]:
    ...
    for stmt in tree.body:
        ...
        if isinstance(target, ast.Name) and isinstance(stmt.value, ast.Dict):
            ...
            _track_subscript_assign(target, stmt.value, result)
    return result


def _track_subscript_assign(
    target: ast.Subscript, value: ast.expr, result: dict[str, dict[str, str]]
) -> None:
...
def _extract_dict_literal(node: ast.Dict) -> dict[str, str]:
