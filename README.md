...
- **AST-based Python analysis** — `.py` files are parsed with Python's `ast` module to catch evasion techniques that regex cannot detect (string concatenation building `eval`, `chr()`-based construction, `getattr` with dynamic names, unsafe deserialization, ROT13 codec usage, and string-splitting evasion where dangerous names or encoded payloads are assembled across multiple variables via concatenation, f-strings with `!s`/`!r` conversions, `join` with lists/generators/`reversed()`/`map(chr)`/`map(str)`, `str.format()`, `%`-formatting with mixed specifiers, dict subscript access, string slicing, `chr(ord())` nesting with arithmetic padding, bytes/bytearray/str/codecs constructor decoding, dynamic dispatch via `globals()`/`vars()`/`__dict__` subscript chains, string multiplication, and cross-function dataflow where functions return string fragments that are assembled into dangerous names at call sites)
