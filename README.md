...
- **AST-based Python analysis** — `.py` files are parsed with Python's `ast` module to catch evasion techniques that regex cannot detect (string concatenation building `eval`, `chr()`-based construction, `getattr` with dynamic names, unsafe deserialization, ROT13 codec usage, and string-splitting evasion where dangerous names or encoded payloads are assembled across multiple variables via concatenation, f-strings, `join`, `str.format()`, or `%`-formatting)
