...
- Tracks dict subscript assignments (`d['key'] = 'val'`), list-index subscript assignments (`parts[0] = 'ev'`), dict literal initializers, `BinOp(Mult)` string repetition, string-list literals (`name[0]`, `name[1]`, `name.__len__`), and `.replace()` chains on tracked variables (via `_resolve_replace_chain_simple`)
- `_handle_string_list_literal(var_name, node, table)` in `_ast_symbol_table_dict_tracker.py` — stores each element of a `Name = [str, str, ...]` assignment as `name[0]` = val, `name[1]` = val, etc., and `name.__len__` = count; enables `parts[0] + parts[1]` BinOp resolution and `''.join([*parts1, *parts2])` star-unpack via `_ast_split_star_unpack`
- `_handle_dict_literal(var_name, node, table)` in `_ast_symbol_table_dict_tracker.py` — stores each `string_key: string_value` pair of a `Name = {k: v}` assignment as composite key `name[key]` = val; enables `dict.pop('key')` resolution
...
- `_ast_symbol_table_assignments.py` imports are deferred inside `_collect_assignments()` to break circular import (`_ast_symbol_table_assignments.py` imports `_Ref` from `_ast_symbol_table.py`)
...
- `_process_nested()` handles nested functions recursively at arbitrary nesting depth: recurses into inner bodies BEFORE routing current-level nonlocal declarations; tracks `own_keys` before recursion so pass-through writes from deeper levels propagate upward past intermediate scopes; routes global writes to module-level result, nonlocal writes to parent scope
...
- `_sub_bodies()` in `_ast_symbol_table_returns.py` is the DRY source — `_ast_symbol_table_self_attrs.py` imports it from there
- `_is_exhaustive_match` is imported from `_ast_terminal_body.py` (moved there in PLAN-036); `_ast_symbol_table_returns.py` imports it to check whether a `match` block has an unguarded wildcard case during implicit-fallthrough analysis

## Terminal Body Detection

- `_ast_terminal_body.py` — shared helper module for terminal-body analysis; created in PLAN-036
- `_is_terminal_body(body: list[ast.stmt]) -> bool` — returns True when every code path through `body` exits scope via `return` or `raise`; conservative (False for unrecognized nodes, empty body, loops, bare `if` without `else`); handles nested `if/else` (both sides must terminate + else must exist), `try/except/finally` (finally alone suffices; otherwise body + all handlers must terminate), `with` (body terminates), exhaustive `match` (all cases terminate)
- `_is_exhaustive_match(node: ast.Match) -> bool` — returns True if last case is an unguarded `MatchAs` with `name=None`; shared between `_ast_terminal_body.py` and (via import) `_ast_split_int_list_tracker.py` and `_ast_symbol_table_returns.py`; moved from `_ast_split_int_list_tracker.py` in PLAN-036
- `break`/`continue` are NOT terminal in this module — they exit loop iteration only, not scope; treating them as terminal would cause false negatives in the int-list tracker
- Used by `_walk_fn_body` in `_ast_split_int_list_tracker.py` to exclude terminal branches from the N-way merge; a branch ending with `return`/`raise` on all paths cannot reach post-branch code and its mutations should not pollute the merged result
...
- `_build_scope_map(tree, *, method_scope=False)` — maps `id(node)` → scope name; now maps class body statements (non-method assignments/expressions) to `ClassName` scope in addition to method body nodes; class-level join/concat/subscript nodes get the correct scope for `ClassName.var` symbol table lookups (fixed in PLAN-032 Part D)
...
- `_ast_split_resolve.py` — expression resolution helpers (`resolve_binop_chain`, `resolve_operand`, `resolve_fstring`, `resolve_expr`, `resolve_call_return`, `resolve_call`); re-exports from `_ast_split_bytes`, `_ast_split_reduce`, `_ast_split_format`, `_ast_split_format_map`, `_ast_split_method_chains`; at 217/350 lines (format_map extracted to `_ast_split_format_map.py`)
...
- `_ast_split_method_chains.py` — replace-chain and case-method-chain resolvers extracted from `_ast_split_resolve.py`; `_resolve_replace_chain()` walks `.replace()` chains up to 20 levels deep; `_resolve_base_string()` resolves the base expression of a chain through all available resolvers; re-exported from `_ast_split_resolve.py`
- `_CASE_METHODS` frozenset + `_is_case_method()` + `_resolve_case_method_chain()` in `_ast_split_method_chains.py` — resolves chained `.lower()`, `.upper()`, `.title()`, `.swapcase()`, `.capitalize()`, `.casefold()` calls (no args) up to 20 levels deep; resolves base via `_resolve_base_string` (constant or symbol-table lookup) then falls back to `resolve_expr`; registered in `_RESOLVERS` before `_is_call`; follows the `_replace` pattern
- `_ast_split_match.py` — dangerous-name matching and encoded-payload bridge for split-evasion results; `_check_dangerous(resolved, file_path, node)` returns a `Finding` for dangerous assembled names or encoded payloads, `None` otherwise; `_NAME_RULE` maps dangerous names to `(rule_id, severity, description_prefix)`; imported by `_ast_split_detector.py`
- `_ast_split_chr.py` — resolves `chr(N)`, `chr(ord('x'))`, `chr(ord('x') + N)`; `_MAX_INT_DEPTH = 50`
- `_ast_split_bytes.py` — resolves `bytearray(b'...').decode()`, `str(b'...', 'utf-8')`, `codecs.decode(b'...', 'utf-8')`; also resolves inline `(bytes.fromhex('XX') + bytes.fromhex('YY')).decode()` via `resolve_fromhex_concat`; `_build_bytes_table` pre-pass tracks `name = bytes.fromhex(...)` variable assignments for split-evasion; `resolve_fromhex_concat` accepts optional `symbol_table` kwarg so `bytes.fromhex(var)` resolves `var` via symbol table lookup (e.g. after `.replace()` chain stripping); symbol_table is `dict[str, str]` and cannot hold bytes objects — tracked-variable fromhex using symbol_table lookup was added in PLAN-032 Part E
- `_ast_split_reduce.py` — resolves `functools.reduce(lambda a,b: a+b, [...])` and `functools.reduce(operator.add/concat, [...])`
- `_ast_split_format.py` — format/%-format resolution; `_scoped_lookup`, `_resolve_join_elements`, `_resolve_subscript_expr`, `_resolve_slice_expr`; `_PERCENT_SPEC_RE` matches all standard %-specifiers with `(?<!%)` lookbehind; `_resolve_format_call` handles both positional and keyword args (keyword args via `node.keywords` loop, rejecting `**kwargs` spreads where `kw.arg is None`); `_resolve_slice_expr` handles `step == -1` reversal (rejects only `step == 0`); at 299/350 lines — effectively frozen
- `_ast_split_format_map.py` — format_map resolution extracted from `_ast_split_resolve.py`; `_resolve_format_map_call` resolves `template.format_map(dict_expr)` via `_extract_dict_literal` or symbol table lookup
- `_ast_split_comprehension.py` — `_resolve_join_call()` dispatches to list/tuple, comprehension, reversed, or map resolvers; calls `_maybe_flatten_starred(arg.elts, symbol_table, scope)` from `_ast_split_star_unpack` before `_resolve_join_elements` to expand `[*parts1, *parts2]` patterns; passes `int_list_table` and `int_list_scope` to `_resolve_map_join`; `_collect_int_list_assigns(tree)` pre-pass collects `Name = [int, ...]` assignments AND tracks `+=`/`.extend()` mutations (module/function/class-body/class-method scope) as `dict[str, list[int]]`; class-level int-lists tracked under `ClassName.varname` keys; delegates statement dispatch to `_handle_int_list_stmt` from `_ast_split_int_list_tracker`; delegates nested function traversal (with global/nonlocal declaration awareness) to `_collect_fn_body` from `_ast_split_int_list_tracker` so mutations inside `global`/`nonlocal`-declaring functions update the correct scope key; `_resolve_comprehension_join()` accepts `int_list_table` kwarg and resolves `chr(c) for c in tracked_var` via `_resolve_tracked_iter()`; also resolves nested 2-generator comprehensions of the form `[chr(c) for row in [[ints],[ints]] for c in row]` by flattening the 2D list-of-lists before dispatching to `_resolve_comprehension_chr`; at 278/350 lines
- `_ast_split_int_list_tracker.py` — int-list mutation tracking; exposes `_SHADOW` sentinel, `_Decls` type alias (`tuple[set[str], set[str]] | None` for optional global/nonlocal declaration sets), `_resolve_scope_key(name, scope, declarations, enclosing_scope)` (central key builder for declaration-aware routing), `_collect_fn_body(fn, scope, enclosing, result)` (recursive nested function traversal with declaration awareness), and `_handle_int_list_stmt(stmt, scope, result, declarations, enclosing_scope)` dispatch entry point; all mutation helpers accept optional `declarations` and `enclosing_scope` parameters so global/nonlocal declarations route writes to the correct scope key; `_handle_assign` tracks `Name = [int, ...]` and `Name = part1 + part2` (BinOp concat of two tracked int-lists via `_resolve_binop_concat`; stores `_SHADOW` for non-int-list values); `_handle_extend_call` handles `name.extend([...])` calls; `_extend_tracked` handles both `+=` and `.extend()` mutations — ignores unknown variables, converts to `_SHADOW` on mixed-type or non-literal arg, concatenates on all-int literal; also resolves `.extend(tracked_var)` where the arg is a tracked int-list Name via `_extend_with_tracked_var`; `_extract_int_list(elts)` returns `list[int] | None`; identity check `existing is _SHADOW` distinguishes shadow markers from legitimate empty lists; `_walk_fn_body` imports `_is_terminal_body` and `_is_exhaustive_match` from `_ast_terminal_body` to exclude terminal branches from the merge (PLAN-036); `_is_exhaustive_match` was previously in this module and was moved to `_ast_terminal_body.py` in PLAN-036; at 291/350 lines
- `_ast_split_star_unpack.py` — star-unpack flattening; `_flatten_starred_list(elts, symbol_table, scope)` replaces `ast.Starred` elements with synthetic `ast.Constant` nodes by looking up `name.__len__` and `name[i]` in the symbol table; `_maybe_flatten_starred(elts, symbol_table, scope)` is a convenience wrapper that skips the call when no starred elements are present; returns original list unchanged if any starred variable is unresolvable; at ~79/350 lines
- `_ast_split_map_resolver.py` — `_resolve_map_join(call, sep, alias_map, *, int_list_table, int_list_scope)` handles `map(chr, [ints])`, `map(lambda c: chr(c), [ints])`, and `map(str, [strs])`; `_effective_map_fn(func_arg, alias_map)` normalizes both `ast.Name` references and `ast.Lambda` (`lambda c: chr(c)`) to a canonical function name; `_is_lambda_chr(node)` validates lambda structure (single arg, body is `chr(arg)`, no vararg/kwonly); tracked int-list variables resolved via `_resolve_tracked_elts`
...
- Dict-collection functions live in `_ast_kwargs_dict_tracker.py` (extracted from `_ast_kwargs_detector.py` to stay under 350 lines); all symbols are re-exported from `_ast_kwargs_detector.py` for backward compatibility
...
- `_build_string_table(body)` in `_ast_kwargs_dict_tracker.py` — local pre-pass over a body collecting `name = 'str'` and `name = 'a' + 'b'` (inline BinOp concat) assignments as `dict[str, str]`; built by `_collect_from_body` and passed to `_extract_dict_literal` for resolving `Name` keys in dict literals (e.g. `key = 'sh' + 'ell'; opts = {key: True}` → `opts['shell'] = True`)
...
- `_ast_exfil_detector.py` — node-level detectors for data exfiltration patterns; registered in `_DETECTORS` in `ast_analyzer.py`; new module was created because `_ast_detectors.py` was at 294/350 lines
...
- `_collect_int_list_assigns` now walks class-level statements (class body tracked under `ClassName.varname`); `class C: codes = [ints]; codes += [more]` is tracked (fixed in PLAN-032 Part A)
- `codes = a + b` (int-list concatenation via BinOp of two tracked int-lists) is now resolved in the pre-pass via `_resolve_binop_concat` (fixed in PLAN-032 Part A)
- `_maybe_flatten_starred` only handles `ast.Starred` where the starred value is a plain `ast.Name`; starred expressions (subscript, attribute, computed) are not expanded (conservative gap remains)
