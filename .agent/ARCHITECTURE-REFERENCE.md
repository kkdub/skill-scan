...
- `_ast_split_star_unpack.py` — star-unpack flattening; `_flatten_starred_list(elts, symbol_table, scope)` replaces `ast.Starred` elements with synthetic `ast.Constant` nodes by looking up `name.__len__` and `name[i]` in the symbol table; `_maybe_flatten_starred(elts, symbol_table, scope)` is a convenience wrapper that skips the call when no starred elements are present; returns original list unchanged if any starred variable is unresolvable; at ~79/300 lines
