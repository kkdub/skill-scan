...
class RepoMap:
    ...
    def __init__(
        ...
        file_reader: Callable[[str], str | None] = default_read_text,
        output_handlers: dict[str, Callable[..., None]] | None = None,
        ...
        self.root = (Path(root) if root else Path.cwd()).resolve()
        ...
        self.map_cache: dict[tuple, tuple[str | None, FileReport]] = {}
    ...
    def _get_ranked_tags_map_uncached(
        ...
        # Binary search for optimal number of tags within token budget
        ...
        while left <= right:
            ...
            if tree_output and self.token_count(tree_output) <= max_map_tokens:
