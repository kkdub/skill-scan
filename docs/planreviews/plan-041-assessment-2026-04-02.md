- [issue]: `patterns=[]` for `AGENT-006` will fail current validation unless additional files are changed outside `allowed_files`. `test_rules_have_patterns` only exempts `{"OBFS-001","EXFIL-008","EXEC-011","PI-030"}` in [tests/unit/test_rule_validation.py](tests/unit/test_rule_validation.py#L31), and the plan explicitly requires `[rules.AGENT-006] patterns=[]` in [plan.yaml](.agent/plans/041/plan.yaml#L135).

- [issue]: Updating `agent_manipulation.toml` without regenerating `RULES.md` will break CI, but `RULES.md` is missing from `allowed_files`. Freshness is enforced in [tests/unit/test_rule_validation.py](tests/unit/test_rule_validation.py#L74), and current catalog only contains `AGENT-001` in [RULES.md](RULES.md#L122).

- [issue]: The utilities audit contains an incorrect factual claim: plan says `agent_manipulation.toml — AGENT-001 stub + patterns=[]` in [plan.yaml](.agent/plans/041/plan.yaml#L30), but AGENT-001 is regex-based with non-empty patterns in [agent_manipulation.toml](src/skill_scan/rules/data/agent_manipulation.toml#L10). That weakens trust in the audit and likely missed dependencies.

- [issue]: Dispatch-order rationale is risky. Plan says run `(suppress_agent_findings, detect_compound_attack)` so AGENT-006 survives filtering in [plan.yaml](.agent/plans/041/plan.yaml#L160). In current engine, `_STRUCTURAL_DETECTORS` is a post-filter pass in [engine.py](src/skill_scan/rules/engine.py#L175). If AGENT-006 is appended after suppression, it bypasses existing agent-context suppression heuristics entirely, increasing doc/test false-positive risk.

- [issue]: Requirement coverage is incomplete for spec intent. Spec includes exfil chain variants such as “write to public location -> push” in [rules-to-build.md](.agent/roadmap/rules-to-build.md#L141), but planned tests focus mainly on URL POST and generic negatives in [plan.yaml](.agent/plans/041/plan.yaml#L260). You can pass acceptance while still breaking key chain variants.
...
- [assumption]: [unverified] — “Suppress-first then detect is correct long-term.” I verified the current dispatch order in [engine.py](src/skill_scan/rules/engine.py#L175), but cannot confirm without implementation/tests whether AGENT-006 false positives remain acceptable when it bypasses `suppress_agent_findings`.

- [claim from plan]: [confirmed by reading src/skill_scan/rules/engine.py] `_STRUCTURAL_DETECTORS` exists and currently contains `suppress_agent_findings` in [engine.py](src/skill_scan/rules/engine.py#L33).

- [claim from plan]: [confirmed by reading tests/unit/test_agent_manipulation_structural.py] Structural test precedent exists and already validates detector registration/dispatch patterns in [test_agent_manipulation_structural.py](tests/unit/test_agent_manipulation_structural.py#L53).

- [claim from plan]: [confirmed by reading src/skill_scan/_package_risk_correlations.py] Correlation logic precedent is table-driven in [\_package_risk_correlations.py](src/skill_scan/_package_risk_correlations.py#L12).
