...
src/skill_scan/           # Production source code
  ...
  _ast_split_detector.py  # Split-evasion detector (detect_split_evasion); reconstructs concat/f-string/join/format/%-format/subscript/Attribute/Call-return payloads; _build_scope_map now walks ClassDef method bodies; _try_resolve_split has call-return fallback for ast.Call nodes
  ...
  _ast_split_resolve.py   # Expression resolution helpers extracted from _ast_split_detector.py (resolve_binop_chain, resolve_operand, resolve_fstring, resolve_expr, resolve_call_return, _resolve_subscript_lookup); Facade Re-export Pattern
...
- `detect_split_evasion(tree, file_path, alias_map, symbol_table) -> list[Finding]` in `_ast_split_detector.py` — reconstructs strings assembled via `BinOp(Add)`, f-string interpolation, `"".join(...)`, `'template'.format(...)`, `'%s%s' % (a, b)`, dict subscript lookups, list-index subscripts, `ast.Attribute` (`self.attr`), and `ast.Call` (function return values via `resolve_call_return()`) using the symbol table; emits EXEC-002 for dangerous names (eval, exec, system, popen), EXEC-006 for dynamic import names (`__import__`, `getattr`); also bridges to decoder for split encoded payloads
...
- Evasion corpus at `tests/fixtures/split_evasion/` — 41 positive (should detect) and 5 negative (should not trigger) Python fixture files; prefix `pos_` / `neg_`; format/%-format fixtures use `UP030`/`UP031`/`UP032` ruff ignores (intentional old-style format syntax); Plan 021 added 7 fixtures: `pos_list_index_concat.py`, `pos_list_index_mutate.py`, `pos_global_overwrite.py`, `pos_nonlocal_overwrite.py`, `pos_class_level_attr.py`, `pos_class_self_attr.py`, `pos_class_cross_method.py`; Plan 022 added 5 fixtures: `pos_call_return_assign.py`, `pos_call_return_inline.py`, `pos_call_return_fstring.py`, `pos_call_return_class.py`, `neg_call_return_safe.py`
