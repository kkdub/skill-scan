...
tests/
  unit/
    ...
    test_int_list_branch_merge.py       # Branch-aware if/else and match/case merge tests for int-list pre-pass (PLAN-035, 26 tests)
...
- `_is_exhaustive_match(node: ast.Match) -> bool` in `_ast_split_int_list_tracker.py` — returns True if the match has a wildcard case (last case pattern is `MatchAs` with `name=None`); used by `_walk_fn_body` to decide whether to include the pre-match snapshot as an extra branch in the N-way merge
