...
src/skill_scan/           # Production source code
  ...
  _ast_split_int_list_tracker.py # Int-list mutation tracking (_SHADOW sentinel, _Decls type alias, _resolve_scope_key, _collect_fn_body, _walk_fn_body, _handle_int_list_stmt, _handle_assign, _handle_extend_call, _extend_tracked, _values_agree, _merge_branches, _is_exhaustive_match)
...
tests/
  unit/
    ...
    test_int_list_branch_merge.py       # Branch-aware if/else and match/case merge tests for int-list pre-pass (PLAN-035, 28 tests)
...
- `_walk_fn_body(body, scope, result, decls=None, enclosing="")` in `_ast_split_int_list_tracker.py` — branch-aware body walker for int-list pre-pass; `ast.If` and `ast.Match` nodes use snapshot-walk-merge so mutually exclusive branches do not contaminate each other; `For`/`While`/`Try`/`With` sub-bodies are still walked sequentially; calls `_merge_branches` to reconcile branch snapshots; non-exhaustive match adds the pre-match snapshot as an extra branch (models "no case matched"); `_sub_bodies` import is deferred inside this function to break circular deps
...
- `_is_exhaustive_match(node: ast.Match) -> bool` in `_ast_split_int_list_tracker.py` — returns True if the match has an unguarded wildcard case (last case pattern is `MatchAs` with `name=None` and `case.guard is None`); used by `_walk_fn_body` to decide whether to include the pre-match snapshot as an extra branch in the N-way merge
...
- `_walk_fn_body` branch-aware merge rule: `If` branches are two-way (if-body vs else-body); `Match` branches are N-way (one per case); non-exhaustive match always adds the pre-match snapshot as an extra branch; `For`/`While`/`Try`/`With` sub-bodies are walked sequentially (not branch-aware) — they are not mutually exclusive; `_merge_branches` uses identity (`is`) for `_SHADOW` and value equality (`==`) for concrete int-lists; this invariant must not be changed without updating the corresponding test_int_list_branch_merge.py tests
...
- `_ast_split_int_list_tracker.py` is at 294/300 lines — 6 lines remaining (tight); any addition requires extracting a helper or removing dead code first
