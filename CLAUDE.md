...
src/skill_scan/           # Production source code
  ...
  _ast_split_detector.py  # Split-evasion detector (detect_split_evasion); reconstructs concat/f-string/join/format/%-format/subscript/Attribute/Call-return/bytes-constructor/reversed()/chr(ord())/dynamic-dispatch payloads; _build_scope_map now walks ClassDef method bodies; _try_resolve_split has call-return fallback for ast.Call nodes; _check_dynamic_dispatch detects globals()/vars()/__dict__ subscript chains; _INTROSPECTION_FUNCS = {"globals", "locals", "vars"}
  ...
  _ast_split_resolve.py   # Expression resolution helpers extracted from _ast_split_detector.py (resolve_binop_chain, resolve_operand, resolve_fstring, resolve_expr, resolve_call_return, _resolve_subscript_lookup, _resolve_chr_call, _resolve_chr_arg, _resolve_int_arg, _resolve_ord_call, _resolve_int_binop, resolve_bytes_constructor, _resolve_bytearray_decode, _resolve_str_bytes, _resolve_codecs_decode, _decode_bytes_encoding_args); Facade Re-export Pattern
...
- `detect_split_evasion(tree, file_path, alias_map, symbol_table) -> list[Finding]` in `_ast_split_detector.py` — reconstructs strings assembled via `BinOp(Add)`, f-string interpolation, `"".join(...)`, `'template'.format(...)`, `'%s%s' % (a, b)`, dict subscript lookups, list-index subscripts, `ast.Attribute` (`self.attr`), `ast.Call` (function return values via `resolve_call_return()`), `reversed()` join, `chr()`/`ord()` chains, bytes-constructor patterns, and dynamic dispatch via introspection subscripts using the symbol table; emits EXEC-002 for dangerous names (eval, exec, system, popen), EXEC-006 for dynamic import names (`__import__`, `getattr`); also bridges to decoder for split encoded payloads
- `_resolve_join_call(node, symbol_table, scope, alias_map)` in `_ast_split_detector.py` — now accepts `alias_map` parameter (threaded from all call sites); dispatches to `_resolve_join_elements` (list/tuple), `_resolve_generator_join` (generator expression), `_resolve_reversed_join` (reversed()), or `_resolve_map_join` (map(chr/str)) depending on join argument type
...
- `_resolve_chr_call(node)` in `_ast_split_resolve.py` — resolves `chr(N)`, `chr(ord('x'))`, and `chr(ord('x') + N)` to a single character; dispatches to `_resolve_chr_arg` → `_resolve_int_arg`; `_resolve_int_arg` handles integer literals, `_resolve_ord_call` (single-char string constant), and `_resolve_int_binop` (Add/Sub/Mult on resolvable integer operands); returns `None` for non-constant arguments (R-IMP003)
- `resolve_bytes_constructor(node, alias_map)` in `_ast_split_resolve.py` — resolves bytes-constructor patterns to a decoded string; dispatches to `_resolve_bytearray_decode` (`bytearray(b'...').decode()`), `_resolve_str_bytes` (`str(b'...', 'utf-8')`), and `_resolve_codecs_decode` (`codecs.decode(b'...', 'utf-8')`); `_decode_bytes_encoding_args` shared helper extracts `(bytes_literal, encoding_str)` from two-arg calls; gates on literal bytes arguments only (R-IMP004); re-exported from `_ast_split_detector.py` (Facade Re-export Pattern)
...
- `_resolve_reversed_join(call, sep, symbol_table, scope)` in `_ast_split_detector.py` — resolves `reversed()` as a join argument; gates to string literals, List/Tuple of tracked elements, and tracked variable names; dispatches to `_resolve_reversed_inner` to get the list of parts, then reverses and joins
- `_resolve_reversed_inner(inner, sep, symbol_table, scope)` in `_ast_split_detector.py` — resolves the inner argument of `reversed()` to a list of characters/parts; handles `str` literal (returns character list), `List`/`Tuple` (joins via `_resolve_join_elements` then splits), and `ast.Name` (symbol table lookup then character list)
